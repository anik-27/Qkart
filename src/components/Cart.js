...
export default class Cart extends React.Component {
  ...
  refreshCart = async () => {
    ...
    // console.log(typeof this.props.history.location.pathname)
    ...
    if(this.state.items.length === 0 && this.props.history.location.pathname === '/checkout') {
      message.error("Cannot access checkout page with an empty cart");
  ...
  getQuantityElement = (item) => {
    ...
    return <div className="cart-item-qty-fixed">Qty: {item.qty}</div>;
      ...
      return (
        <InputNumber min={1} max={10} defaultValue={item.qty} onChange={(qty) => {this.pushToCart(item.productId, qty, false)}} ></InputNumber>
      )
    }
  };
    ...
    return (
      ...
      >
        ...
        {/* TODO: CRIO_TASK_MODULE_CART - If props.checkout is not set, display a checkout button*/}
          {this.props.checkout ? null : (<Button type="primary" onClick={() => {this.state.items.length ? this.props.history.push('/checkout') : message.info("You must add items to cart first")}} icon={<ShoppingCartOutlined />}>
      Checkout
    </Button>)}
