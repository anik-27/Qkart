...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    if (this.validateResponse(errored, response, "delete an address")) {
      if (response) {
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
        ...
        let currentBalance = parseInt(localStorage.getItem('balance')) - this.cartRef.current.calculateTotal();
  ...
  async componentDidMount() {
    let isUsernameExist = localStorage.getItem('username');
    let isTokenExist = localStorage.getItem('token');

    if(isTokenExist && isUsernameExist) {
      await this.getProducts();
      await this.getAddresses();
      this.setState({balance: parseInt(localStorage.getItem('balance'))});
    } else {
      ...
      <>
        ...
        <div className="checkout-container">
          <Row>
            ...
            <Col xs={{ span: 24, order: 2 }} sm={{order: 2}} md={{ span: 18, order: 1 }}>
                            ...
                            <div className="address">
                              <Radio.Button value={index}>
                                <div className="address-box">
                                  ...
                                  <Button
                                    type="primary"
                                    onClick={() => {this.deleteAddress(this.state.addresses[this.state.selectedAddressIndex]._id)}}
                                  >
                                    Delete
                                  </Button>
                                </div>
            ...
            {/* Display the cart */}
            <Col xs={{ span: 24, order: 1 }} md={{span: 6, order: 2}}  sm={{ order: 1 }}  className="checkout-cart">
              <div>
