...
class Search extends React.Component {
   ...
   */
  getProducts = async () => {
    const productData = await this.performAPICall();
   ...
   */
   ...
   componentDidMount() {
     this.getProducts();
     if(localStorage.getItem('username')) this.setState({loggedIn: true});
   }
   ...
   */
  search = (text) => {
    const nameBasedFiltered = this.products.filter((product) => {
      return product.name.toUpperCase().includes(text.toUpperCase());
    })
    const categoryBasedFiltered = this.products.filter((product) => {
      return product.category.toUpperCase().includes(text.toUpperCase());
    })

    if(nameBasedFiltered.length) this.setState({filteredProducts: [...nameBasedFiltered]});

    else if(categoryBasedFiltered.length) this.setState({filteredProducts: [...categoryBasedFiltered]});
      ...
      <>
        ...
        <Header history={this.props.history}>
          {/* TODO: CRIO_TASK_MODULE_PRODUCTS - Display search bar in the header for Products page */}
            <Input.Search placeholder="Search" onSearch={(text) => {this.search(text)}} onChange={(e) => {this.debounceSearch(e)}} enterButton />
          ...
          {this.state.loggedIn && this.products.length && (
            ...
            >
              <div>
                ...
                {this.state.loggedIn ? <Cart history={this.props.history} products={this.products} token={localStorage.getItem('token')}  ref={this.myRef} /> : null}
              </div>
            </Col>
          )}
        </Row>
